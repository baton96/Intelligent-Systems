...
def trapezoid(val,bottomLeft,upperLeft,upperRight,bottomRight):
	if(val<bottomLeft): return 0
	elif(val<upperLeft): return (val-bottomLeft)/(upperLeft-bottomLeft)
	elif(val<upperRight): return 1.0
	elif(val<bottomRight): return (bottomRight-val)/(bottomRight-upperRight)
	else: return 0

def pi(val,bottomLeft,upperLeft,upperRight,bottomRight):
	if(val<(upperLeft+upperRight)/2): return 1/(1+math.exp(k*((bottomLeft+upperLeft)/2-val)))
	else: return 1/(1+math.exp(k*(val-(upperRight+bottomRight)/2)))
